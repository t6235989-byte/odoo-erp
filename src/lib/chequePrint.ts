...
// Positions are in millimetres, measured from the LEFT and BOTTOM edges of
// the cheque leaf (bottom is used instead of top because it's unaffected by
// how far the leaf is pushed into the printer tray).
...
export const DEFAULT_CHEQUE_TEMPLATE: ChequeTemplate = {
  ...
  figuresX: 160, figuresXEnd: 190, figuresY: 60,
  fontSize: 11,
};
...
export function buildChequeHTML(t: ChequeTemplate, data: { payee: string; amount: number; date: string }): string {
  ...
  const dateDigits = data.date ? data.date.split('-').reverse().join('').split('') : []; // DDMMYYYY
  // date input value is YYYY-MM-DD -> reverse to DD MM YYYY then split to chars
  const ddmmyyyy = data.date ? (() => { const [y, m, d] = data.date.split('-'); return (d + m + y).split(''); })() : [];

  const dateBoxesHtml = t.dateBoxesX.map((x, i) =>
    `<div style="position:absolute;left:${x}mm;bottom:${t.dateY}mm;font-size:${t.fontSize}pt;font-weight:bold">${ddmmyyyy[i] || ''}</div>`
  ).join('');
  ...
  <style>
    * { margin:0;padding:0;box-sizing:border-box; }
    ...
    @media print { @page { size:${t.pageWidth}mm ${t.pageHeight}mm; margin:0; } }
    @media screen { body{background:#eee;padding:20px} .cheque{box-shadow:0 0 8px rgba(0,0,0,.3)} }
    .cheque { position:relative; width:${t.pageWidth}mm; height:${t.pageHeight}mm; background:#fff; }
  </style></head><body>
  <div class="cheque">
    <div style="position:absolute;left:${t.payeeX}mm;bottom:${t.payeeY}mm;font-size:${t.fontSize}pt;font-weight:bold;white-space:nowrap">${data.payee.toUpperCase()}</div>
    ${dateBoxesHtml}
    <div style="position:absolute;left:${t.words1X}mm;bottom:${t.words1Y}mm;font-size:${t.fontSize}pt;font-weight:bold;white-space:nowrap">${line1}</div>
    ${line2 ? `<div style="position:absolute;left:${t.words2X}mm;bottom:${t.words2Y}mm;font-size:${t.fontSize}pt;font-weight:bold;white-space:nowrap">${line2}</div>` : ''}
    <div style="position:absolute;left:${t.figuresX}mm;bottom:${t.figuresY}mm;font-size:${t.fontSize}pt;font-weight:bold;white-space:nowrap">${data.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</div>
  </div>
...
// up the real cheque leaf against the printed sheet under a light.
export function buildCalibrationTestHTML(t: ChequeTemplate): string {
  let grid = '';
  for (let x = 0; x <= t.pageWidth; x += 10) grid += `<div style="position:absolute;left:${x}mm;bottom:0;top:0;width:1px;background:#ccc"></div><div style="position:absolute;left:${x + 1}mm;bottom:1mm;font-size:6pt;color:#999">${x}</div>`;
  for (let y = 0; y <= t.pageHeight; y += 10) grid += `<div style="position:absolute;bottom:${y}mm;left:0;right:0;height:1px;background:#ccc"></div><div style="position:absolute;bottom:${y + 1}mm;left:1mm;font-size:6pt;color:#999">${y}</div>`;
  return buildChequeHTML(t, { payee: 'TEST PAYEE NAME', amount: 12345.5, date: new Date().toISOString().split('T')[0] })
    .replace('<div class="cheque">', `<div class="cheque">${grid}`);
}
